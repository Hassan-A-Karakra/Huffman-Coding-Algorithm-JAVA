...
public class Huffman {
	...
	public static void compress(File file) throws IOException {
		...
		do {
			for (int i = 0; i < size; i++) {
				...
				if (index < 0)
					index += 256;
				if (tmp[index] == 0)
					huffCodeArraySize++; // Counting number of different
		...
		huffCodeArray = new HuffCode[huffCodeArraySize]; // building the
															// HuffmanCodeArray

		for (int i = 0; i < 256; i++)
			if (tmp[i] != 0) {
				huffCodeArray[tracker++] = new HuffCode((char) i, tmp[i]);// Number
																			// of
																			// Frequency
				nbChar += tmp[i];
				tmp[i] = 0;
		...
		for (int i = 0; i < tracker; i++) {
			builder.append(byteToBinaryString(outbuffer[i]));
	...
	public static String deCompress(File infile) {
		String newFileName = "";
		try {
			...
			originalFileName = String.valueOf(tmp, 0, tracker).trim();
			...
			if (dotIndex != -1) {
				newFileName = originalFileName.substring(0, dotIndex) + "_decompressed"
						+ originalFileName.substring(dotIndex);
			} else {
				newFileName = originalFileName + "_decompressed";
			}

			String desktopPath = "C:\\Users\\coolnet\\OneDrive\\ÓØÍ ÇáãßÊÈ";
			File outputFile = new File(desktopPath, newFileName);
